...
import com.jlm.translator.databinding.PopLanguageThreeSelectorBinding
...
sealed class ThreeSelectorParams {
    class InputParam(
 ...
 * 有三个语言选择器
 * */
class ThreeLangSelectorPop(
    val context: Context,
    var selectLangMode: LanguageModeEnum,
    val inputParams: ThreeSelectorParams.InputParam,
    val callback: ThreeSelectorParams.callbackData.() -> Unit
): BasePopupWindow(context) {
    val TAG: String = javaClass.name
    private val vb: PopLanguageThreeSelectorBinding

    var sourceLanguage: Language = inputParams.sourceLanguage
    ...
    init {
        setContentView(R.layout.pop_language_three_selector)
        ...
        //绑定视图
        vb = PopLanguageThreeSelectorBinding.bind(contentView)
        //初始化数据
        initLangData()
        setHeaderUi()
        initList()
    ...
    private fun setHeaderUi() {
        ...
        //更新选择的语言text
        vb.tvFromLanguage.text = sourceLanguage.getLangName()
...
//                    }
                ...
                onClick(R.id.item_view) {
                    getModelOrNull<Language>()?.run {
                        when (selectLangMode) {
                            LanguageModeEnum.LANG_SOURCE -> { //源
                                sourceLanguage = this
                                vb.tvFromLanguage.text = this.getLangName()
                                // 发送callback更新
                                ThreeSelectorParams.callbackData(selectLangMode, sourceLanguage).callback()
                            }
                            ...
                            LanguageModeEnum.LANG_TARGET_LEFT -> { //目标-左
                                ...
                                //回调
                                ThreeSelectorParams.callbackData(selectLangMode, targetLeftLanguage).callback()
                            }
                            ...
                            LanguageModeEnum.LANG_TARGET_RIGHT -> { //目标右
                                ...
                                //回调
                                ThreeSelectorParams.callbackData(selectLangMode, targetRightLanguage).callback()
                            }
        ...
        when(selectLangMode) {
            LanguageModeEnum.LANG_SOURCE -> {
                ...
                for (item in vb.rvLanguage.models!!) {
                    if (item is Language) {
                        if (item.getItemKey() == sourceLanguage?.getItemKey()) {
    ...
    private fun isChecked(language: Language): Boolean {
        var check = false
        when (selectLangMode) {
            LanguageModeEnum.LANG_SOURCE -> if (language.getItemKey() == sourceLanguage.getItemKey()) check = true
