...
import com.jlm.translator.databinding.ActTranslationRecordsDetailsBinding
import com.jlm.translator.databinding.ItemListenRecordsDetailsBinding
...
/**短语对话记录详情*/
@Route(path = Rt.TranslationRecordsDetailsAct)
class TranslationRecordsDetailsAct : BaseActCompat() {

    private val vb by binding<ActTranslationRecordsDetailsBinding>()
    ...
    private fun initView() {
        ...
        //初始化列表数据
        vb.rvListenRecord.linear()
            .dividerSpace(5)
            .setup {
                addType<TranslateHistory>(R.layout.item_listen_records_details)
                onBind {
                    with(getBinding<ItemListenRecordsDetailsBinding>()) {
                        with(getModel<TranslateHistory>()) {
                            tvContent.text = sourceText
                            tvTranslationContent.text = targetText
                            //判断是否有第二个输出语言
                            if (targetSecondText.isNullOrEmpty()) {
    ...
    private fun initListener() {
        //返回或取消选择模式
        vb.ivBack.doClick {
            if (vb.rvListenRecord.bindingAdapter.toggleMode) {//选择模式下，取消选择
                vb.rvListenRecord.bindingAdapter.toggle()
            } else {
                finish()
            }
        }
        //右侧菜单或全选
        vb.ivAction.doClick {
            if (vb.rvListenRecord.bindingAdapter.toggleMode) {//选择模式下，全选与反选
                vb.rvListenRecord.bindingAdapter.checkedAll(!vb.rvListenRecord.bindingAdapter.isCheckedAll())
            } else {
                vb.rvListenRecord.bindingAdapter.toggle()
            }
        }
        ...
        vb.includeTranslationRecordsBottomOperation.tvIncludeOutput.doClick {
            ChooseOutputTypePop(this@TranslationRecordsDetailsAct) { type ->
                if (type == 1) {
                    val sb = StringBuilder()
                    sb.append("Time:${time}")
                    sb.append("\n")
                    sb.append("========================")
                    vb.rvListenRecord.bindingAdapter.getCheckedModels<TranslateHistory>().forEach { history ->
                    ...
                    sb.toString().insertClipboard(this@TranslationRecordsDetailsAct)
                    toast("已复制到剪切板")
                    // 导出成功后退出选择模式
                    vb.rvListenRecord.bindingAdapter.toggle()
                } else {
        ...
        vb.includeTranslationRecordsBottomOperation.tvIncludeDel.doClick {
            showConfirmWindow(
                this@TranslationRecordsDetailsAct,
                "提示",
                "您要删除这些记录吗？",
                leftStr = "取消"
            ) {
                val checkedList = vb.rvListenRecord.bindingAdapter.getCheckedModels<TranslateHistory>()
    ...
    private fun changeListEditable(bindingAdapter: BindingAdapter) {
        ...
        if (toggleMode) {
            ...
            vb.ivAction.isSelected = vb.rvListenRecord.bindingAdapter.isCheckedAll()
        } else {
            vb.ivBack.setImageResource(R.drawable.tools_nav_icon_back_black_def)
            vb.ivAction.setImageResource(R.drawable.tools_nav_icon_menu)
        }
    ...
    override fun backPressed() {
        if (vb.rvListenRecord.bindingAdapter.toggleMode) {
            vb.rvListenRecord.bindingAdapter.toggle()
        } else {
            super.backPressed()
        }
