...
import cn.chawloo.base.ext.bool
import cn.chawloo.base.ext.dp
import cn.chawloo.base.ext.float
import cn.chawloo.base.ext.int
...
class NumberProgressBar @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    override fun onRestoreInstanceState(state: Parcelable?) {
        if (state is Bundle) {
            mTextColor = state.int(INSTANCE_TEXT_COLOR)
            mTextSize = state.float(INSTANCE_TEXT_SIZE)
            mReachedBarHeight = state.float(INSTANCE_REACHED_BAR_HEIGHT)
            mUnreachedBarHeight = state.float(INSTANCE_UNREACHED_BAR_HEIGHT)
            mReachedBarColor = state.int(INSTANCE_REACHED_BAR_COLOR)
            mUnreachedBarColor = state.int(INSTANCE_UNREACHED_BAR_COLOR)
            setMaxProgress(state.int(INSTANCE_MAX))
            setProgress(state.int(INSTANCE_PROGRESS))
            mPrefix = state.string(INSTANCE_PREFIX)
            mSuffix = state.string(INSTANCE_SUFFIX)
            mIfDrawText = state.bool(INSTANCE_TEXT_VISIBILITY)
            super.onRestoreInstanceState(state.parcelable(INSTANCE_STATE))
            return
        } else {
